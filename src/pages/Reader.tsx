...
export default function Reader() {
  ...
  const currentChapterIdx =
    comic?.chapters?.findIndex((ch) => ch.id === chapterId) ?? -1;
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen text-stone-500 bg-brand-dark">
        <h1 className="text-2xl font-display mb-4">
          Шастир эсвэл бүлэг олдсонгүй
        </h1>
        <Link
          to="/library"
          className="text-brand-gold uppercase tracking-widest hover:underline"
        >
          Номын сан руу буцах
        </Link>
      </div>
    ...
    <div className="bg-brand-dark min-h-screen relative flex flex-col overflow-x-hidden w-full">
      ...
      <div className="sticky top-0 z-40 bg-brand-dark/95 backdrop-blur-md border-b border-brand-gold/20 h-16 flex items-center justify-between px-2 sm:px-4">
        <div className="flex items-center gap-2 sm:gap-4 min-w-0">
          <Link
            to="/library"
            className="p-2 text-brand-cream/40 hover:text-brand-gold shrink-0"
          >
            <ChevronLeft size={24} />
          </Link>
        ...
        <div className="flex items-center gap-2 sm:gap-4 shrink-0">
          <div className="hidden md:flex items-center bg-brand-charcoal rounded-xl p-1 border border-brand-gold/10">
            <button
              ...
              className="p-2 text-brand-cream/40 hover:text-brand-gold group"
            >
              <ZoomOut size={18} />
            </button>
            <span className="text-[10px] font-mono w-10 text-center text-brand-cream/40">
              {zoom}%
            </span>
            <button
      ...
      <div className="flex-grow flex justify-center bg-brand-dark py-4 sm:py-8 px-0 sm:px-4">
        ...
        >
          ...
          <div className="w-[calc(100%-2rem)] sm:w-full max-w-2xl py-12 sm:py-20 flex flex-col items-center border-t border-brand-gold/10 mt-12 bg-gradient-to-b from-brand-dark to-brand-charcoal p-6 sm:p-12 rounded-2xl">
            <p className="text-brand-gold font-display text-base sm:text-lg tracking-[0.3em] font-medium mb-8 sm:mb-12 uppercase italic">
              Бүлэг дууслаа
            </p>
            ...
            <div className="grid grid-cols-2 w-full gap-4 sm:gap-8">
              ...
              >
                <ChevronLeft
                  size={24}
                  className="text-brand-gold group-hover:-translate-x-2 transition-transform sm:w-8 sm:h-8"
                />
                <span className="text-[9px] sm:text-[10px] uppercase tracking-widest text-brand-cream/40 text-center">
                  Өмнөх БҮЛЭГ
                </span>
              </button>
              ...
              <button
                disabled={
                  currentChapterIdx === (comic.chapters?.length ?? 0) - 1
                }
                onClick={handleNext}
                className="flex flex-col items-center gap-3 sm:gap-4 py-4 sm:py-8 border border-brand-gold/10 hover:border-brand-gold/50 group transition-all disabled:opacity-30 disabled:pointer-events-none rounded-2xl"
              >
          ...
          <div className="w-[calc(100%-2rem)] sm:w-full max-w-2xl py-8 px-4 sm:px-8 bg-brand-charcoal border border-brand-gold/10 rounded-2xl mt-12 text-left">
            ...
            <div className="space-y-4 mb-8 max-h-[400px] overflow-y-auto pr-2">
              {comments.length === 0 ? (
                <p className="text-xs uppercase font-sans text-stone-500 text-center py-6 italic">
                  Энэ шастирт сэтгэгдэл бичигдээгүй байна.
                </p>
              ) : (
                comments.map((c) => (
                  <div
                    key={c.id}
                    className="border border-brand-gold/5 bg-brand-dark/40 p-4 rounded-xl flex justify-between gap-4"
                  >
                    <div className="flex gap-4">
                      {c.userPhoto ? (
                        <img
                          src={c.userPhoto}
                          alt={c.userName}
                          className="w-8 h-8 rounded-full border border-brand-gold/10 shrink-0"
                          referrerPolicy="no-referrer"
                        />
                      ) : (
                        <div className="w-8 h-8 rounded-full bg-stone-850 border border-white/5 flex items-center justify-center font-bold text-stone-500 text-xs shrink-0">
                          {c.userName?.charAt(0)}
                        </div>
                      )}
                      <div>
                        ...
                        <p className="text-stone-300 font-serif text-xs italic">
                          "{c.text}"
                        </p>
                      </div>
                    </div>
                    ...
                    {user &&
                      (user.uid === c.userId ||
                        user.email === "naranbadrakh1013@gmail.com" ||
                        user.email === "haitan.admin@gmail.com") && (
                        <button
                          onClick={() => handleDeleteComment(c.id)}
                          className="text-stone-600 hover:text-red-500 transition-colors self-start"
                        >
                          <Trash2 size={14} />
                        </button>
                      )}
            ...
            {user ? (
              ...
              >
                <div className="flex-grow">
                  <label className="text-[9px] uppercase tracking-wider font-bold text-brand-gold/70 block mb-1">
                    Сэтгэгдэл үлдээх
                  </label>
                  <input
                    type="text"
                ...
                <button
                  type="submit"
                  className="bg-brand-gold text-brand-dark p-3 rounded-xl hover:bg-brand-amber transition-colors shrink-0"
                >
                  <Send size={16} />
                </button>
              </form>
              ...
              <div className="bg-brand-dark/30 border border-brand-gold/10 p-4 rounded-xl text-center">
                <p className="text-stone-400 text-xs font-serif italic mb-4">
          ...
          <>
            ...
            >
              <div className="p-5 sm:p-8 border-b border-brand-gold/10 flex justify-between items-center">
                <h3 className="text-xl sm:text-2xl font-display font-bold text-white tracking-widest uppercase italic">
                  Бүлгүүд
                </h3>
                <button
                  onClick={() => setSidebarOpen(false)}
                  className="text-brand-cream/40 hover:text-brand-gold p-1"
                >
              ...
              <div className="flex-grow overflow-y-auto px-3 sm:px-4 py-6 sm:py-8">
                <div className="flex flex-col gap-3 sm:gap-4">
                  {(comic.chapters || []).map((ch) => (
                    ...
                    >
                      <span
                        className={`text-2xl sm:text-4xl font-display font-black transition-colors ${
                          ch.id === chapter.id
                            ? "text-brand-gold"
                            : "text-stone-800 group-hover:text-stone-700"
                        }`}
                      >
                      ...
                      <div className="min-w-0 flex-1">
                        <h4
                          className={`font-bold transition-colors mb-1 text-xs sm:text-sm truncate ${
                            ch.id === chapter.id
                              ? "text-brand-gold"
                              : "text-stone-300"
                          }`}
                        ...
                        <span className="text-[9px] sm:text-[10px] uppercase tracking-widest text-stone-500 block">
                          {ch.releaseDate}
                        </span>
                      </div>
                    </button>
