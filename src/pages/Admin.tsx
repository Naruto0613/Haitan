...
export default function Admin() {
  ...
  const [comicStatus, setComicStatus] = useState<"Үргэлжилж буй" | "Дууссан">(
    "Үргэлжилж буй",
  );
  ...
  const handleSaveComic = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast(
        editingComic
          ? "Шастир амжилттай засагдлаа!"
          : "Шинэ шастир амжилттай нэмэгдлээ!",
      );
  ...
  const handleDeleteComic = async (id: string) => {
    if (!confirm("Энэ шастирыг устгах уу? Бүх бүлгүүд хамт устах болно!"))
      return;
    try {
    ...
    try {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        const storageRef = ref(
          storage,
          `pages/${Date.now()}__${i}_${file.name}`,
        );
        const snapshot = await uploadBytes(storageRef, file);
        const url = await getDownloadURL(snapshot.ref);
        urls.push(url);
  ...
  const handleAddChapter = async (e: React.FormEvent) => {
    ...
    const existingIdx = updatedChapters.findIndex(
  ...
  const handleToggleFeedbackRead = async (
    ...
    try {
      ...
      showToast(
        !currentStatus
          ? "Санал хүсэлтийг уншсан гэж тэмдэглэлээ."
          : "Санал хүсэлтийг шинэ уншаагүй гэж тэмдэглэлээ.",
      );
    } catch (err) {
      console.error(err);
    }
        ...
        >
          ...
          <div className="flex justify-center mb-6">
            <div className="w-16 h-16 border border-brand-gold/40 rounded-full flex items-center justify-center bg-brand-gold/5">
              <span className="text-brand-gold font-display font-black text-2xl tracking-tighter">
                ᠪᠣᠭᠳᠠ
              </span>
            </div>
          </div>

          <h2 className="text-3xl font-display font-medium text-white uppercase tracking-widest mb-3">
            Хоршооны Алтан Хаалга
          </h2>
          ...
          <div className="border border-brand-gold/10 p-6 rounded-xl bg-brand-dark/40 mb-8">
            <p className="text-stone-400 font-serif text-sm leading-relaxed italic">
              "Энэхүү хэсэгт зөвхөн эрх бүхий түүхэн шүүмжлэгчид болон админ
              удирдлагууд нэвтрэх боломжтойг анхаарна уу."
            </p>
          </div>
          ...
          {!user ? (
            ...
            <div className="space-y-6">
              <div className="flex items-center gap-4 bg-red-950/20 border border-red-500/20 text-red-400 p-4 rounded-xl text-left text-xs">
                ...
                <span>
                  Харамсалтай нь, таны нэвтэрсэн хаяг ({user.email}) нь удирдах
                  эрхтэй админ биш байна. Түр хүлээнэ үү эсвэл доорх товчийг
                  дарж гарна уу.
                </span>
              </div>
              <button
      ...
      <header className="border-b border-brand-gold/10 bg-[#111111] px-4 sm:px-8 py-4 sm:py-6 relative">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="flex items-center gap-4">
            <span className="text-3xl text-brand-gold font-display font-black leading-none">
              ᠪᠣᠭᠳᠠ
            </span>
            <div className="border-l border-brand-gold/20 pl-4">
              <h1 className="text-xl sm:text-2xl font-display font-medium text-white uppercase tracking-widest">
                Удирдах Их Өргөө
              </h1>
              <p className="text-brand-gold text-[9px] uppercase tracking-[0.3em] font-sans font-bold">
                Хааны архив ба Хяналтын зөвлөл
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4 bg-[#1a1a1a]/80 border border-brand-gold/10 p-2 pl-4 pr-6 rounded-2xl shadow-xl w-full md:w-auto">
            ...
            <div className="text-left">
              <p className="text-xs font-sans font-bold text-white leading-none mb-1">
                {user.displayName || "Администратор"}
              </p>
              <span className="text-[10px] font-sans text-brand-cream/50">
                {user.email}
              </span>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-12 flex flex-col lg:flex-row gap-6 sm:gap-12">
        ...
        <aside className="lg:w-80 shrink-0 bg-[#111111] border border-brand-gold/15 rounded-2xl p-6 relative h-fit shadow-xl shadow-black/60">
          <div className="mb-6 pb-4 border-b border-brand-gold/10 text-left">
            <span className="text-[10px] text-stone-500 uppercase tracking-widest font-sans font-bold">
              Хуудасны цэс
            </span>
          </div>
          ...
          <nav className="flex flex-col gap-2">
            ...
            >
              ...
              <ChevronRight
                size={14}
                className={
                  activeTab === "comics" ? "opacity-100" : "opacity-30"
                }
              />
            </button>
            ...
            >
              ...
              <div className="flex items-center gap-2">
                ...
                <ChevronRight
                  size={14}
                  className={
                    activeTab === "feedback" ? "opacity-100" : "opacity-30"
                  }
                />
              </div>
            </button>
            ...
            >
              ...
              <ChevronRight
                size={14}
                className={activeTab === "users" ? "opacity-100" : "opacity-30"}
              />
            </button>
            ...
            >
              ...
              <ChevronRight
                size={14}
                className={
                  activeTab === "comments" ? "opacity-100" : "opacity-30"
                }
              />
            </button>
          </nav>
          ...
          <div className="mt-12 p-4 border border-brand-gold/10 bg-brand-gold/5 rounded-xl text-left">
            <span className="text-brand-gold font-display font-bold text-[10px] tracking-wider block uppercase mb-1">
              Салхины сургаал
            </span>
            <p className="text-[11px] leading-relaxed text-stone-500">
              Шастирыг арвижуулан, ардын хүсэлтийг сайтар уншиж, түүхийн зөв
              урсгалыг хадгалж ажиллана уу.
            </p>
          </div>
        </aside>
                ...
                <div className="bg-[#111111] border border-brand-gold/15 rounded-2xl p-4 sm:p-8 shadow-xl text-left">
                  ...
                  >
                    <div className="space-y-4">
                      <div>
                        <label className="text-[10px] uppercase font-bold text-stone-400 tracking-wider font-sans block mb-1">
                          Туульсын Нэр
                        </label>
                        <input
                          type="text"
                    ...
                    <div className="space-y-4 flex flex-col justify-between">
                      <div className="space-y-4">
                        ...
                        <div>
                          <label className="text-[10px] uppercase font-bold text-stone-400 tracking-wider font-sans block mb-1">
                            Нүүр зураг (Cover art) байршуулах
                          </label>
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                              ...
                              {isUploadingCover ? (
                                ...
                                <>
                                  <ImageIcon
                                    ...
                                    size={24}
                                  />
                                  <span className="text-[10px] font-sans text-stone-400 font-bold uppercase">
                                    Зураг сонгох
                                  </span>
                                </>
                              )}
                            ...
                            <div className="bg-[#1c1c1c] border border-brand-gold/10 rounded-xl p-3 flex flex-col justify-between">
                              <span className="text-[8px] uppercase font-bold text-stone-500 block mb-1">
                                Эсвэл холбоос URL суулгах:
                              </span>
                              <input
                                type="text"
                                value={comicCoverURL}
                                onChange={(e) =>
                                  setComicCoverURL(e.target.value)
                                }
                        ...
                        <div className="flex gap-4 items-center bg-brand-dark/50 border border-brand-gold/10 p-3 rounded-xl mt-2">
                          ...
                          <div className="text-left text-[11px] font-sans text-stone-400 truncate">
                            <p className="font-bold text-brand-gold">
                              Зургийн холбоос идэвхтэй байна:
                            </p>
                            <span className="truncate block opacity-60">
                              {comicCoverURL}
                            </span>
                          </div>
                        </div>
                ...
                <div className="bg-[#111111] border border-brand-gold/15 rounded-2xl p-4 sm:p-8 shadow-xl text-left">
                  ...
                  >
                    <div className="space-y-4">
                      <div>
                        ...
                        <select
                          ...
                          onChange={(e) => {
                            ...
                            const selectedComicObj = comicsList.find(
                              (c) => c.id === e.target.value,
                            );
                            if (selectedComicObj) {
                              setChapterNumber(
                                (selectedComicObj.chapters?.length || 0) + 1,
                              );
                            }
                          }}
                          className="w-full bg-[#1c1c1c] border border-brand-gold/20 focus:border-brand-gold outline-none p-3 text-sm text-stone-300 rounded-xl font-sans"
                      ...
                      <div>
                        <div className="flex justify-between items-center mb-1">
                          <label className="text-[10px] uppercase font-bold text-stone-400 tracking-wider font-sans block">
                            Бараалах хуудасны зургууд
                          </label>
                          <span className="text-[9px] font-mono text-stone-500">
                            Шинэ мөрөөр тусгаарласан URL оруулах буюу байршуулна
                          </span>
                        </div>
                        <textarea
                          rows={6}
                    ...
                    <div className="flex flex-col justify-between">
                      <div className="space-y-4">
                        <label className="text-[10px] uppercase font-bold text-stone-400 tracking-wider font-sans block leading-none">
                          Олон Хуудас устгагчтай ачаалагч
                        </label>
                        ...
                        <label className="flex flex-col items-center justify-center border-2 border-dashed border-brand-gold/15 hover:border-brand-gold/40 h-40 bg-[#1c1c1c] rounded-2xl cursor-pointer text-center group p-6 transition-all">
                          {isUploadingPages ? (
                            <div className="flex flex-col items-center">
                              <Loader2
                                className="text-brand-gold animate-spin mb-2"
                                size={28}
                              />
                              <span className="text-xs font-sans text-brand-gold font-bold uppercase animate-pulse">
                                Зургуудыг ачаалж байна (Storage)...
                              </span>
                            </div>
                          ) : (
                            <>
                              <ImageIcon
                                size={36}
                                className="text-stone-500 group-hover:text-brand-gold transition-colors mb-3"
                              />
                              <span className="text-xs font-sans font-black uppercase text-stone-300 group-hover:text-white transition-colors">
                                Олон зургууд Сонгох
                              </span>
                              <span className="text-[10px] text-stone-500 font-sans mt-1">
                                Хуудасны зургуудыг зэрэг сонгоод байршуулна уу
                              </span>
                            </>
                          )}
                          <input
                        ...
                        <div className="bg-brand-dark/40 border border-brand-gold/10 p-4 rounded-xl text-xs font-sans text-stone-400 space-y-2">
                          <p className="font-bold text-white uppercase text-[10px] tracking-widest text-brand-gold">
                            Зөвлөмж
                          </p>
                          <p>
                            1. Төрөлх санах ойдоо зургуудаа байршуулаад, хаягийг
                            шууд өргөтгөл дээр харах боломжтой.
                          </p>
                          <p>
                            2. Хэрэв өөрсдийн түүхэн хуудсууд байршсан Unsplash
                            буюу бусад холбоос байгаа бол хажуугийн хэсэгт
                            гараар мөр мөрөөр нь нэмж оруулж болно.
                          </p>
                        </div>
                      </div>
                        ...
                        <div className="flex-grow flex flex-col justify-between truncate">
                          <div>
                            <div className="flex justify-between items-start gap-2">
                              <h4 className="text-lg font-display font-bold text-white truncate group-hover:text-brand-gold transition-colors">
                                {comic.title}
                              </h4>
                            ...
                            <p className="text-[10px] font-sans text-stone-500 mb-2">
                              Зохиогч: {comic.author} • Төлөв: {comic.status}
                            </p>
                            <p className="text-xs text-stone-400 font-serif italic line-clamp-2 leading-relaxed mb-2">
                              "{comic.tagline}"
                            </p>
                            <span className="text-[10px] text-brand-gold font-sans font-bold uppercase">
                              {comic.chapters?.length || 0} бүлэг хадгалагдсан
                            </span>
                          </div>
            ...
            {activeTab === "feedback" && (
              ...
              >
                <div className="flex justify-between items-center border-b border-brand-gold/15 pb-4">
                  <div>
                    <h2 className="text-xl font-display font-bold text-white uppercase tracking-widest">
                      📬 САНИЛ ХҮСЭЛТИЙН ХАЙРЦАГ ({feedbacksList.length})
                    </h2>
                    <p className="text-[10px] font-sans text-brand-gold uppercase tracking-wider mt-1">
                      Ардын дуу хоолой, санал сэтгэгдэл
                    </p>
                  </div>
                  {unreadFeedbackCount > 0 && (
                ...
                {feedbacksList.length === 0 ? (
                  <div className="py-24 text-center border-2 border-dashed border-brand-gold/10 rounded-2xl">
                    <Mail
                      className="mx-auto text-brand-gold/20 mb-4 animate-bounce"
                      size={48}
                    />
                    <p className="text-stone-500 font-serif italic text-sm">
                      Санал хүсэлтийн хайрцаг одоогоор хоосон байна.
                    </p>
                  </div>
                ) : (
                      ...
                      >
                        ...
                        <div className="flex flex-col md:flex-row md:items-start justify-between gap-4 mb-4">
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              <h4 className="text-base font-bold text-white font-sans leading-none mb-1">
                                {fb.userName || "Зочин хэрэглэгч"}
                              </h4>
                              <p className="text-xs font-sans text-stone-500">
                                {fb.email || "Имэйл байхгүй"}
                              </p>
                          ...
                          <div className="flex items-center gap-1.5 bg-brand-dark/40 border border-brand-gold/10 px-3 py-1.5 rounded-lg w-fit">
                            <span className="text-[10px] font-sans font-bold text-brand-gold uppercase mr-1">
                              Үнэлгээ:
                            </span>
                            {[...Array(5)].map((_, i) => (
                              <Star
                        ...
                        <div className="flex justify-end gap-4 mt-4 pt-3 border-t border-brand-gold/5 text-xs font-sans font-bold uppercase">
                          <button
                            onClick={() =>
                              handleToggleFeedbackRead(fb.id, fb.isRead)
                            }
            ...
            {activeTab === "users" && (
              ...
              >
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-brand-gold/15 pb-4">
                  <div>
                    <h2 className="text-xl font-display font-bold text-white uppercase tracking-widest">
                      👥 БҮРТГЭЛТЭЙ ХЭРЭГЛЭГЧИД ({filteredUsers.length})
                    </h2>
                    <p className="text-[10px] font-sans text-brand-gold uppercase tracking-wider mt-1">
                      Түүхэн клубын гишүүд
                    </p>
                  </div>
                  ...
                  <div className="relative group max-w-sm w-full">
                    <Search
                      className="absolute left-3 top-1/2 -translate-y-1/2 text-stone-500 group-focus-within:text-brand-gold transition-colors"
                      size={16}
                    />
                    <input
                      type="text"
                ...
                {filteredUsers.length === 0 ? (
                  <div className="py-24 text-center border-2 border-dashed border-brand-gold/10 rounded-2xl">
                    <Users
                      className="mx-auto text-brand-gold/10 mb-4"
                      size={48}
                    />
                    <p className="text-stone-500 font-serif italic text-sm">
                      Таны хайлтод таарах хэрэглэгч олдсонгүй.
                    </p>
                  </div>
                ) : (
                            ...
                            return (
                              ...
                              >
                                ...
                                <td className="py-4 px-6 text-center">
                                  {isUserAdmin ? (
                                    <span className="text-[10px] px-2.5 py-1 bg-brand-gold/15 border border-brand-gold/30 rounded-full font-bold text-brand-gold uppercase">
                                      Админ
                                    </span>
                                  ) : (
                                    <span className="text-[10px] px-2.5 py-1 bg-stone-850 text-stone-500 rounded-full font-medium">
                                      Гишүүн
                                    </span>
                                  )}
                                </td>
            ...
            {activeTab === "comments" && (
              ...
              >
                <div className="border-b border-brand-gold/15 pb-4">
                  <h2 className="text-xl font-display font-bold text-white uppercase tracking-widest">
                    💬 СЭТГЭГДЭЛ МОДЕРАТОР ({commentsList.length})
                  </h2>
                  <p className="text-[10px] font-sans text-brand-gold uppercase tracking-wider mt-1">
                    Нийтлэгдсэн хэлэлцүүлэг, шүүмжүүдийг цензурдах өрөө
                  </p>
                </div>
                ...
                {commentsList.length === 0 ? (
                  <div className="py-24 text-center border-2 border-dashed border-brand-gold/10 rounded-2xl">
                    ...
                    />
                    <p className="text-stone-500 font-serif italic text-sm">
                      Хэвлэгдсэн сэтгэгдэл одоогоор байхгүй байна.
                    </p>
                  </div>
                ) : (
                        ...
                        <div className="flex items-start gap-4">
                          ...
                          <div className="text-xs">
                            <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mb-1">
                              <span className="font-bold font-sans text-brand-cream">
                                {comment.userName}
                              </span>
